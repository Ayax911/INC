...
class TrainModel():
	...
	Args:
		options (dict): Dictionary containing training configuration options.
	"""
	...
	def __init__(self, options:dict):
		...
		if(options.test):
			self.model.load_state_dict(torch.load(os.path.join(dir_save_models, "Best_Model.pth")))
			self.model.eval()
		
		# Get loss function. If class balance is True, use weighted loss
		...
		# Get loaders
		loader 				= Loader(data_dir=options.data_clinic_path)
	...
	def validation(self, plot=False):
		...
		with torch.no_grad():
			...
			for batch_idx, data in enumerate(self.val_loader):
				...
				logits 		= self.model(inputs)
				probs 		= torch.softmax(logits, dim=1)  # Apply sigmoid to get probabilities
		...
		#self.scheduler.step(self.epoch_stats["F1-Score"])
		self.epoch_stats["epoch"] 	= self.epoch
		...
		self.scheduler.step(self.epoch_stats["Val_F1-Score"])
