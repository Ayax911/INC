...
def print_options(options: argparse.Namespace) -> None:
    ...
    experiment_directory = os.path.join(options.path_project, options.experiment_name)
...
def save_code(options: argparse.Namespace) -> None:
    """
    Save code to the results folder.

    Args:
        - options: (Namespace), input options.
    """
    # Define the path to the directory where the code will be saved
    exp_dir = os.path.join(options.path_project, options.experiment_name, 'Code')
